...
Lime2D is a lightweight monochromatic graphics engine with built-in Lua 5.5.0 interpreter.
...
This script can be run by saving it as a `.lua` file and placing it in the same folder as `lime2d.exe`.
...
**Unavailable Lua Libraries:** The `os` and `io` standard libraries are intentionally disabled for security. Use `lime.filesystem` for persistent storage.

**Available Lua Libraries:** `base`, `table`, `string`, `math`, `utf8`, and `coroutine` are available.
...
2. Drag and drop the `.txt` file onto `lime2d.exe`
...
Lime2D applications can be distributed as a single, self-contained EXE file by "fusing" your scripts with the engine. A fused executable contains the standard `lime2d.exe` concatenated with a zip archive containing your Lua script file(s).
...
2. **Fuse the archive with lime2d.exe**: `copy /b lime2d.exe+script.zip FusedApp.exe`
