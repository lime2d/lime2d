...
static void getIntFromArrayTable(lua_State* L, int tindex, lua_Integer i, lua_Integer* out)
{
    lua_geti(L, tindex, i);
...
int LuaHost::l_graphics_lsets(lua_State* L)
{
    ...
    for (size_t i = 1; i <= n; i += 4)
    {
        ...
        getIntFromArrayTable(L, 1, (lua_Integer)i + 0, &x1);
        getIntFromArrayTable(L, 1, (lua_Integer)i + 1, &y1);
        getIntFromArrayTable(L, 1, (lua_Integer)i + 2, &x2);
        getIntFromArrayTable(L, 1, (lua_Integer)i + 3, &y2);
...
int LuaHost::l_graphics_lsetsc(lua_State* L)
{
    ...
    {
        lua_Integer x, y;
        getIntFromArrayTable(L, 1, (lua_Integer)i, &x);
        getIntFromArrayTable(L, 1, (lua_Integer)i + 1, &y);
        if (x < 0 || x >= w || y < 0 || y >= h)
            return luaL_error(L, "lime.graphics.lsetsc: out of bounds (%d,%d)-(%d,%d)", px, py, x, y);
...
int LuaHost::l_require(lua_State* L)
{
    ...
    int status = luaL_loadbufferx(L, chunk.data(), chunk.size(), chunkname.c_str(), "t");
    if (status != LUA_OK)
    {
        lua_remove(L, cacheIndex);
        return lua_error(L);
...
void LuaHost::loadFusedScript(const std::string& archivePath)
{
    ...
    if (status != LUA_OK)
    {
        std::string err = lua_tostring(L, -1) ? lua_tostring(L, -1) : "luaL_loadbufferx failed";
        lua_pop(L, 1);
        throw std::runtime_error(err);
    }
